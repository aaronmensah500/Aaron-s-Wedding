            ...
            {tab === "details" && (
              <div className="adm-stack">
                ...
                <p className="adm-hint">
                  ...
                  <code>PUBLIC_MAPTILER_API_KEY</code> in <code>.env</code> to load MapTiler raster tiles for the interactive pin map. Standard map ids (e.g. <code>streets-v4</code>) work on free keys; <strong>custom MapTiler Cloud map UUIDs</strong> often return 403 unless your plan allows rendered custom maps — use a standard style in the editor or upgrade. Without the key, an Esri World Street Map basemap is used instead. If tiles show 403 with MapTiler, add your origins to the key’s <strong>HTTP referrer</strong> allowlist (include <code>http://localhost:4324</code> for local dev). Map id can come from the embed URL or the field below. Driving directions use <strong>Google Maps</strong> only (Apple Maps does not support directions in Ghana).
                </p>
                <AdminTextField
